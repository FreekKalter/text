...
func (b *Writer) updateWidth() {
	// Discard terminal color escape codes
	ansiEscBeginRegex := regexp.MustCompile(`^(\x1b\[\d+m)+`)
	ansiEscEndRegex := regexp.MustCompile(`(\x1b\[0m)+$`)
	stripped := ansiEscBeginRegex.ReplaceAllString(ansiEscEndRegex.ReplaceAllString(string(b.buf.Bytes()[b.pos:b.buf.Len()]), ""), "")

	b.cell.width += utf8.RuneCount([]byte(stripped))
	b.pos = b.buf.Len()
}
